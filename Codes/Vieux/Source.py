...
def distance(alpha, L, R, Rs, z): #distance between the gamma photon and the source IR

    beta = np.arcsin(np.sin(alpha) * R/L)
...
R = np.array([2, 5, 8.5, 10]) *  3.085678e21#distance between the gamma-source and the IR source (cm)
Rs = 30 * 3.085678e21 #distance between us and the IR source (cm)
alpha = np.pi/2.0 #angle between the direction of the Sun-IR source and the direction gamma-source-IR souce (radian)
...
for k in range(len(R)):
    ...
    L = np.sqrt(R[k]**2 + Rs**2 - 2*R[k]*Rs*np.cos(alpha)) #distance to the source (cm)

    b = R * Rs/L * np.sin(alpha) #impact parameter (cm)
    z = np.linspace(0, L, 100) #position along the line of sight (cm)
    zb = np.sqrt(R[k]**2 - b[k]**2) #position along the line of sight closely the source IR (cm)
    d = distance(alpha, L, R[k], Rs, z)
    ...
    for i in range(len(E)):
        ...
        for j in range(len(z)):
            ...
            integrand = f(u_in, b[k], d[j], eps, T, z[j], zb, E[i])
    ...
    plt.plot(E_tev, np.exp(-tau), label = "at %.2f" %R_kpc)
...
#plt.text(100, 0.98,u'R = %.2f kpc, Rs = %.2f kpc \n ' r'$\alpha$' ' = %.2f rad' %(R_kpc, Rs_kpc, alpha))
